...
const ListaDeTarefas = ({ tarefas, setTarefas }) => {
  ...
  const editList = (index) => {
    ...
    const novaDescricao = prompt("Edite o item da lista:", tarefa.descricao);
    const novoEstado = prompt("Edite a complexidade do item:", tarefa.estado);

    if (novaDescricao !== null && novoEstado !== null) {
      const novasTarefas = tarefas.map((t, i) => 
        i === index ? { ...t, descricao: novaDescricao, estado: novoEstado } : t
      );
      setTarefas(novasTarefas);
    }
  };
    ...
    border-separate border-spacing-0 rounded-xl">
      <thead className='bg-slate-200 dark:bg-slate-800'>
        <tr>
          ...
          <th className="text-left text-slate-800 dark:text-slate-100">Item</th>
          <th className="text-left text-slate-800 dark:text-slate-100">Complexidade</th>
          <th></th>
          <th></th>
        </tr>
      </thead>
      <tbody>
        {tarefas.map((tarefa, index) => (
          <tr key={tarefa.id}>
            ...
            <td className="text-right w-px cursor-pointer" onClick={() => editList(index)}>✏️</td>
